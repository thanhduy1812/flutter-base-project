...
class ComboRoomPickerView extends BaseView<HotelRoomPickerViewModel> {
  ...
  const ComboRoomPickerView({super.key, required super.viewModel, this.onRemove, this.onNotifyChanged});
          ...
          Card(
            ...
            child: Column(
              children: [
                ...
                SizedBox(
                  ...
                  child: Padding(
                    ...
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        const Text("Sơ sinh(< 2 tuổi)"),
                        PassengerPicker(
                          defaultValue: viewModel.infant,
                          max: viewModel.adult,
                          min: 0,
                          onPressed: (newValue) {
                            onNotifyChanged?.call();
                            setState(() {
                              viewModel.infant = newValue;
                              if (newValue < viewModel.roomInfants.length + 1) {
                                viewModel.roomInfants.removeLast();
                              } else {
                                viewModel.roomInfants.add(HotelRoomChildAge(position: newValue));
                              }

                              if (newValue == 0) {
                                viewModel.roomInfants = [];
                              }
                            });
                          },
                        ),
                ...
                viewModel.infant == 0 ? const SizedBox() : const Divider(),
                viewModel.infant == 0
                    ? const SizedBox()
                    : ListView.builder(
                        shrinkWrap: true,
                        physics: const NeverScrollableScrollPhysics(),
                        itemCount: viewModel.roomInfants.length,
                        itemBuilder: (context, index) {
                          var item = viewModel.roomInfants[index];
                          return AnimatedSize(
                            duration: const Duration(milliseconds: 500),
                            child: InkWell(
                              onTap: () {
                              ...
                              child: Padding(
                                padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
                                child: GtdInfoRow(
                                  leftText: "Tuổi sơ sinh ${item.position}",
                                  rightText: item.age != null ? "${item.age} tuổi" : "Vui lòng chọn",
                                  rightIcon: Icon(
                                    Icons.chevron_right,
                                    color: AppColors.mainColor,
  ...
  void _showAgePicker(
      ...
      GtdCallback<int>? onConfirm,
      int? initialAge,
      required int minValue,
      required int maxValue}) {
    List<int> childRangeAges = Iterable<int>.generate(maxValue + 1)
        .map((e) => e)
        .where((element) => (element >= minValue && element <= maxValue))
        .toList();
    int selectedIndex = (initialAge ?? 0) < minValue ? 0 : (initialAge ?? minValue) - minValue;
              ...
              return Column(
                ...
                children: [
                  ...
                  GtdButton(
                    ...
                    borderRadius: 25,
                    onPressed: (value) {
                      onConfirm?.call(selectedIndex + minValue);
                      context.pop();
                    },
                  )
                ],
