  ...
  _emailSectionVisible() {
    buttonEnableStream = Rx.combineLatest4(
      ...
      (lastName, firstName, emailSectionValid, tab) {
        ...
        if (lastName != GtdInputValidation.valid || firstName != GtdInputValidation.valid || !emailSectionValid) {
          return false;
        } else {
          return true;
        }
  ...
  _phoneSectionVisible() {
    buttonEnableStream = Rx.combineLatest4(
      ...
      (lastName, firstName, phoneSectionValid, tab) {
        ...
        if (lastName != GtdInputValidation.valid || firstName != GtdInputValidation.valid || !phoneSectionValid) {
          return false;
        } else {
          return true;
        }
  ...
  onSubmit({
    ...
    final response = await GtdAuthenticationRepository.shared.register(
      RegisterRequest(
        ...
        firstName: registerForm.control(RegisterFormControlNames.firstName.value).value,
        lastName: registerForm.control(RegisterFormControlNames.lastName.value).value,
        password: registerForm.control(RegisterFormControlNames.password.value).value,
      ),
    );
    GtdLoading.hide();
